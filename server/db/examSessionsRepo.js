...
function listExamSessions() {
  const db = getDb();
  return db.prepare('SELECT * FROM exam_sessions ORDER BY scheduled_at DESC').all();
}

// 当前时间点是否处于某个已安排的模考锁定窗口内
  ...
  return db
    .prepare(
      `SELECT * FROM exam_sessions
       WHERE status != 'cancelled' AND scheduled_at <= ? AND locked_until > ?
       ORDER BY scheduled_at DESC LIMIT 1`
    )
    .get(now, now);
}

function cancelExamSession(id) {
  const db = getDb();
  db.prepare("UPDATE exam_sessions SET status = 'cancelled' WHERE id = ?").run(id);
}
...
function createMockFullSession({ scheduledAt } = {}) {
  const db = getDb();
  ...
  db.prepare(
    `INSERT INTO exam_sessions (id, type, scheduled_at, locked_until, status, progress)
     VALUES (?, 'mock_full', ?, ?, 'scheduled', ?)`
  ).run(id, startAt.toISOString(), lockedUntil.toISOString(), JSON.stringify(progress));
  return getMockSession(id);
}

function getMockSession(id) {
  const row = getExamSession(id);
  if (!row) return null;
...
function getPendingMockSession() {
  const db = getDb();
  const row = db
    .prepare(
      `SELECT * FROM exam_sessions WHERE type = 'mock_full' AND status = 'scheduled' ORDER BY scheduled_at DESC LIMIT 1`
    )
    .get();
  if (!row) return null;
  return { ...row, progress: JSON.parse(row.progress || 'null') };
...
function findStageAttemptIds(stage, stageStartedAt) {
  if (stage === 'writing') {
    const candidates = listAttempts({ module: 'writing', since: stageStartedAt, limit: 10 });
    const bySubtype = {};
  ...
  const candidates = listAttempts({ module: stage, since: stageStartedAt, limit: 5 });
  return candidates[0] ? [candidates[0].id] : [];
}

function advanceMockSession(id) {
  const db = getDb();
  ...
  const attemptIds = findStageAttemptIds(stage, progress.stageStartedAt);
