...
import { getRandomTask2, gradeTask2, type WritingItem } from '@/lib/api'
...
const PHASES: ExamPhase[] = [
  { key: 'analyze', label: '审题', seconds: 3 * 60 },
  { key: 'outline', label: '大纲', seconds: 5 * 60 },
  { key: 'writing', label: '写作', seconds: 30 * 60 },
...
export default function Task2Exam() {
  ...
  const handleSubmit = async () => {
    ...
    timer.pause()
    try {
      const data = await gradeTask2({ itemId: item.id, essayText, durationSec: totalElapsed })
      setResult(data)
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-8">
      ...
      <div className="flex items-center justify-between mt-4 mb-2">
        <h1 className="text-2xl font-semibold">写作 Task 2</h1>
      ...
      <Card className="mb-4">
        ...
        <CardContent>
          {item ? <p className="leading-relaxed">{item.content.prompt}</p> : <p className="text-muted-foreground">加载中…</p>}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="pt-6">
          <textarea
            ...
            className="w-full min-h-[360px] resize-y rounded-md border border-border bg-transparent p-3 text-sm leading-relaxed focus:outline-none focus:ring-2 focus:ring-ring"
          />
          <div className="flex items-center justify-between mt-3">
            <Badge variant={wordCount >= 250 ? 'default' : 'secondary'}>{wordCount} / 250 词</Badge>
            <Button onClick={handleSubmit} disabled={grading || wordCount < 50}>
